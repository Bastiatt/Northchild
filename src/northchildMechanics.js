...
export function computeNorthchildResult(animals, hfCount, gsCount) {
  const errors = [];
  ...
  Object.entries(animals).forEach(([k, v]) => {
    if (!Number.isFinite(v) || v < 0) errors.push(`${k} must be >= 0.`);
    if (v > 4) errors.push(`${k} must be <= 4.`);
  });

  const variant = hfCount === 5 ? "HF" : gsCount === 5 ? "GS" : "Base";

  // Axis scores (raw)
  const axisScores = {};
  const axisPrimaryAnimal = {};
  ...
  const sortedRaw = Object.entries(axisScores)
  ...
  const dampFactor = closeness >= SOFTCAP_THRESHOLD ? 1 - SOFTCAP_BETA * closeness : 1;

// Adjusted axis scores
  const adjustedAxisScores = {};
  ...
  const fateRows = FATES.map((f) => {
    const fateScore = (adjustedAxisScores[f.axisA] ?? 0) + (adjustedAxisScores[f.axisB] ?? 0);
    ...
    const composite = fateScore * 1000 + primaryMaxPoints * 10 + (31 - f.id) / 1000;
...
export function getResultImagePath(id, variant) {
  const padded = id.toString().padStart(2, "0");
  const folder = variant === "HF" ? "highflame" : variant === "GS" ? "gravesong" : "base";
